    ...
    def __init__(self, file_paths: List[str], chunk_size=1024, overlap=128, 
        ...
        if not tokenizer_path.exists():
            raise FileNotFoundError(
                ...
                "请确保 bert-base-chinese 文件夹与 text_pretrain_data.py 在同一目录"
            )
...
class PretrainDataProcessor:
    ...
    def __init__(self, file_paths: List[str], chunk_size=1024, overlap=128,
                 batch_size=4, num_workers=4, mlm_config=None, max_chunks=None):
        self.batch_size = batch_size
        
        self.dataset = TextChunkDataset(file_paths, chunk_size, overlap, max_chunks=max_chunks)
        ...
        if mlm_config is None:
            mlm_config = {
                'mask_prob': Config.data.MASK_PROBABILITY,
            }
        self.mlm_preprocessor = MLMPreprocessor(self.dataset.tokenizer, **mlm_config)
        
        # DataLoader 配置（根据 num_workers 动态调整）
        dataloader_kwargs = {
    ...
    def process_batch(self, token_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        embedded = self.embedding(masked_input)  # 使用 masked_input 而不是原始 token_ids
