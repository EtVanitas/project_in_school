...
class MainModel(nn.Module):
    ...
    def _process_inputs(self, x_list, updates, records):
        ...
        Args:
            x_list: List[Tensor[m, n]] - 输入列表
            updates: List[(act_idx, x, score)] - 用于更新短期记忆
            records: List[(x, remain)] - 用于记录长期轨迹（包含 short 和 long）
        """
        for x in x_list:
            # 遍历 72 个 activate 类别
    ...
    def _handle_high_activation(self, x, act_idx, score, input_name, updates, records, record_x=None, record_remain=None):
        ...
        Args:
            x: Tensor[m, n] - 经过 remain 增强的输入
            act_idx: int - 激活类别索引
            score: float - 激活值
            input_name: str - 输入名称（用于调试）
            updates: List - 短期记忆更新列表
            records: List - 长期记忆记录列表
            record_x: Tensor - 用于记录轨迹的原始 x（可选）
            record_remain: Tensor - 用于记录轨迹的 remain（可选）
        """
        # 预占位检查：池子是否已满且分数不够高
