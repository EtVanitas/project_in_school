...
def main():
    ...
    test_file = list(sorted(data_dir.glob('wiki_*')))[4]
    ...
    print(f"开始训练...")
    print(f"  每文件训练轮数：{Config.train.EPOCHS_PER_CHUNK}")
...
def main_resume():
    ...
    trainer.train_streaming(
        ...
        num_epochs_per_file=Config.train.EPOCHS_PER_CHUNK,
        batch_size=None,  # 使用配置的默认值
        start_file_idx=0,  # 总是从第 0 个文件开始（只有一个文件）
        max_chunks=None  # 使用全部文本块
    )
