...
async def prejudge_node(state: dict) -> dict:
    ...
    # 最近几条历史作指代消解语境（与 agent 节点短期记忆同源，窗口更小）
    history: list[dict] = []
    try:
        recent = await notes.build_short_term(conv_id, window=4) if conv_id else []
        history = [m for m in recent if m.get("id") != state.get("pending_msg_id")]
    except Exception:
        logger.warning("预判取历史失败（仅用当前问题判断）", exc_info=True)
    step = _emit_step("prejudge", "小模型预判")
    decision = await llm.prejudge(state["question"], history)
    if decision is None:
        _emit_obs(step, "prejudge", False, "预判不可用，走默认检索链路")
        return {"need_retrieval": True, "rewritten_query": ""}
    need, rewrite = decision["need_retrieval"], decision["query"]
    _emit_obs(step, "prejudge", True,
              (f"需检索" + (f"，检索词改写为「{rewrite}」" if rewrite else "（未改写）"))
              if need else "判定无需检索，跳过预检索注入")
...
async def retrieve_node(state: dict) -> dict:
    ...
    step = _emit_step("retrieve", f"混合检索「{(rewrite or query)[:40]}」")
    try:
        if rewrite:
            chunks = await asyncio.to_thread(
                documents.retrieve_top_chunks_dual, pdf_path, f"{query} {rewrite}", rewrite, TOP_CANDIDATES)
        else:
