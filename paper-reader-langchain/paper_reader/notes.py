"""笔记与记忆：笔记文件存储 + 对话记忆（短期窗口/总结整理/注入）。

笔记即记忆文件：「总结整理」把对话压缩为结构化 Markdown 追加到该论文笔记；
问答时读取笔记注入 Agent 作历史背景。短期记忆为最近 SHORT_WINDOW 条消息原文窗口。
设计原则：每篇论文独立文件，仅做文件级增删改查，不做跨文档记忆召回。
"""
...
async def summarize_conversation(conversation_id: int) -> dict:
    """把整段对话压缩为结构化总结并追加到该论文笔记（「总结整理」入口）。

    Returns:
        {"summary": 总结正文, "arxiv_id": 笔记键, "title": 论文标题}
    Raises:
    ...
    summary = await llm.summarize_dialog(title, messages, conversation_id=conversation_id)
    key = str(doc.get("arxiv_id") or doc.get("id"))
    try:
        await asyncio.to_thread(append_summary, key, title, summary)
    except Exception as e:
        raise RuntimeError(f"总结已生成，但写入笔记失败：{e}") from e
    logger.info("对话 %d 已整理为笔记（%s，%d 字符）", conversation_id, key, len(summary))
    return {"summary": summary, "arxiv_id": key, "title": title}
