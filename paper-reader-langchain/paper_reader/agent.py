...
async def run_agent(doc: dict, history: list[dict], question: str, selected_text: str,
    ...
    msgs: list = [SystemMessage(_SYSTEM_AGENT.format(title=doc.get("title") or "未命名", profile=profile_block))]
    for m in history:
        if m["role"] == "user":
    ...
    while step < MAX_STEPS:
        ...
        for ci, call in enumerate(tool_calls):
            ...
            if total_calls >= config.MAX_TOOL_CALLS:  # 总量护栏：不再执行新工具，空观测补齐消息后强制收敛
    ...
    # 步数耗尽 / 超时 / 总调用量达上限：强制基于已收集信息收敛（不带工具再问一次）
    logger.info("Agent 达到步数上限或超时（step=%d, total_calls=%d），强制收敛最终回答", step, total_calls)
